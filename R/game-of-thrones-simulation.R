...
got.key <- read.csv("https://raw.githubusercontent.com/harsh2201/GOTNetworkAnalysis/refs/heads/master/book1-nodes.csv")
got <- read.csv("https://raw.githubusercontent.com/harsh2201/GOTNetworkAnalysis/refs/heads/master/book1-edges.csv")
g.got <- graph_from_edgelist(as.matrix(got[, 1:2]), directed = FALSE)
E(g.got)$weight <- got$weight
g.adj <- as.matrix(as_adjacency_matrix(g.got, attr = "weight"))
...
#got quantile_n = 0.167
boot.result <- localboot_sparce(g.got, 3000, dist_func = dist_ASE, quantile_n = 0.167, weighted = TRUE)
...
save(ret, file = "data/got-strength.Rdata")
...
save(ret, file = "data/got-eigen.Rdata")
